...
```mermaid
flowchart TD
    subgraph BISON_JD["BISON CRM-Manager Responsibilities (Job Spec)"]
        T1["1. Full-Lifecycle Campaigns<br>(Onboarding to Re-Engagement)"]
        T2["2. Multichannel Orchestration<br>(Email, Push, IAM, Banners)"]
        T3["3. Automated Customer Journeys<br>(Behavioral & Market Triggers)"]
        T4["4. Segmentation & Personalization<br>(Portfolio & Liquid Logic)"]
        T5["5. Testing & Statistical Rigor<br>(A/B Z-Tests & Unit Tests)"]
    ...
    subgraph ENGINE["Live Faizan CRM Engine Implementation"]
        ...
        T4 --> E4["Braze Liquid Conditional Blocks<br>(BTC/ETH/EUR + Bilingual DE/EN)"]
        T5 --> E5["Two-Proportion Z-Test Engine<br>(p = 0.0018, 5 Automated Python Tests)"]
        T6 --> E6["Growth Squad Matrix & Pre-Launch QA<br>(Snowflake SQL, Deep-Links & DOI Ledger)"]
    end
```
